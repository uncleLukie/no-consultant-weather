...
export function IOSInstallPrompt({ isDarkMode }: IOSInstallPromptProps) {
      ...
      <div className="max-w-screen-md mx-auto flex flex-col gap-4">
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <h3 className="font-bold text-lg mb-1">Install NC Weather</h3>
            <p className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Install this app on your home screen for the best experience. It's fast, full-screen, and looks just like a native app!
            </p>
          </div>
          <button 
            onClick={() => setShowPrompt(false)}
            className={`p-1 rounded-full hover:bg-opacity-20 ${isDarkMode ? 'hover:bg-gray-400 text-gray-400' : 'hover:bg-gray-600 text-gray-500'}`}
            aria-label="Close"
          >
