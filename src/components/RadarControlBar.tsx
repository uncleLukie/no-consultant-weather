...
export default function RadarControlBar({
  ...
  const containerClass = isVertical
    ? `flex flex-col gap-2 p-2 ${isDarkMode ? 'bg-gray-800/50' : 'bg-gray-50/50'} rounded-lg border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`
    : `w-full px-1.5 py-2 md:px-2 md:py-3 flex flex-wrap items-center justify-center gap-1 md:gap-2 ${isDarkMode ? 'bg-gray-800/50' : 'bg-gray-50/50'} rounded-t-lg border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`;

  return (
    <div className={containerClass}>
      {/* Range Buttons */}
      <div className={`flex ${isVertical ? 'flex-col' : 'items-center'} gap-1 md:gap-2`}>
        {ranges.map((range) => {
          ...
          return (
            ...
            >
              <span className={`flex items-center ${isVertical ? 'justify-between' : 'gap-0.5'}`}>
      ...
      {/* Divider */}
      {hasDoppler && (
        <div className={`${isVertical ? 'h-px w-full' : 'h-8 w-px'} ${isDarkMode ? 'bg-gray-600' : 'bg-gray-300'}`} />
      )}
      ...
      {hasDoppler && (
        ...
        >
          <span className={`flex items-center ${isVertical ? 'justify-between' : 'gap-0.5 md:gap-1.5'}`}>
            <svg
            ...
            <span className={`${isVertical ? '' : 'hidden sm:inline'} whitespace-nowrap`}>Doppler</span>
          </span>
        </button>
      )}
    </div>
